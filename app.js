...
// Netlify Functions를 통한 WordPress API 호출
function api(path) {
...
async function getTotalPostsCount() {
  console.log('getTotalPostsCount 호출 (Netlify Functions 사용)...');
  
  try {
    // Netlify Functions를 통한 호출 (per_page=1로 최소 데이터만)
    const proxyUrl = api('posts?per_page=1&page=1');
    console.log('  → Netlify Functions 호출:', proxyUrl);
    ...
    console.log('  → 응답 헤더 - Total:', totalFromProxy, 'TotalPages:', totalPagesFromProxy);
...
async function loadHomeChunk(page = 1) {
  ...
  state.homeLoadPromise = new Promise(async (resolve, reject) => {
    try {
      ...
      if(totalPages === 0) {
        console.log('헤더 정보 없음 - Netlify Functions로 재시도...');
        ...
        if(count && count.totalPages > 0) {
          ...
          console.log('Netlify Functions 호출 성공 - TotalPages:', totalPages, 'Total:', count.total);
        }
      }
